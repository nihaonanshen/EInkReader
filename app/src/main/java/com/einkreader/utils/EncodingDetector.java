 ...
 * ★ 速度优化：采样大小从 64KB 降至 8KB（8KB 已足够区分常见中文编码）
 */
public class EncodingDetector {
    ...
    /**
     * 从字节数组中检测编码（用于 TxtParser 已读入全文件的场景）
     */
    ...
    /**
     * 快速统计预筛：只确认 UTF-8，不会误判为 GBK
     */
    private static boolean isUtf8ByStats(byte[] data, int len) {
        int utf8Count = 0;
        int limit = Math.min(len, 32768);
